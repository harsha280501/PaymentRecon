    ...
    <x-selection.checkboxes :selectionHas="$selectionHas">
        ...
        <div class="row mb-4">
            <div class="col-lg-9">
                <ul class="nav nav-tabs justify-content-start" role="tablist">
                    ...
                    <li class="nav-item">
                        <a @click.prevent="$wire.showBankDrop = false"
                            class="nav-link @if ($showBankDrop == false) active tab-active @endif"
                            data-bs-toggle="
                tab" data-bs-target="#axis" role="tab"
                            style="font-size: .9em !important" href="#">
                            Without BankDrop
                        </a>
                    </li>

                    <li class="nav-item">
                        <a @click.prevent="$wire.showBankDrop = true"
                            class="nav-link @if ($showBankDrop == true) active tab-active @endif"
                            data-bs-toggle="
                tab" data-bs-target="#axis" role="tab"
                            style="font-size: .9em !important" href="#">
                            With BankDrop
            ...
            <div class="col-lg-12">
                <div>
                    <div class="d-flex d-flex-mob gap-2" style="flex-wrap: wrap; ">
                        ...
                        <x-filters.extensions.dropdown_one keys="bank" initialValue="SELECT A BANK"
                            :dataset="$banks" />
                        <div class="">
                            <div wire:ignore>
                                <select id="select102-dropdown" wire:model="matchStatus" style="width: 220px;"
                                    class="custom-select select2 form-control searchField w-mob-100 mb-1"
                                    data-live-search="true" data-bs-toggle="dropdown" style="height: 8px !important">
                        ...
                        <div>
                            <div
                                style="display: flex; gap: 1em; justify-content: space-between; align-items: center; height: fit-content; width:  100%;">
                                <template x-if="!selection.length">
                                    <button data-bs-toggle="modal" data-bs-target="#import-modal"
                                        ...
                                        style="background: lightgreen; color: #fff;">Import</button>
                                </template>

                                <template x-if="!selection.length">
                                    <button type="button" class="btn mb-1 py-2 mt-1"
                                        style="background: lightcoral; color: #000;"
                                        @click.prevent="$wire.importExcelExport()">
                                        Export Excel (import) </button>
                                </template>

                                <template x-if="!selection.length">
                                    <button type="button" class="btn mb-1 py-2 mt-1"
                                        style="background: #3682cd; color: #fcf8f8;" @click.prevent="$wire.export()">
                                        Export Excel </button>
                                </template>

                                <template x-if="selection.length">
                                    <button type="button" class="btn btn-danger mb-1 py-2 mt-1 text-dark"
                                        @click.prevent="() => {
                                    confirmAction(() => {
                                        loading = true
        ...
        <div class="d-flex justify-content-center mt-4" style="flex-direction: column;">
            ...
            <div class="d-flex justify-content-center flex-column flex-md-row gap-md-4 align-items-center">
                <p class="mainheadtext">Total Record: <span
                        class="blackh">{{ isset($_totals[0]->TotalCount) ? $_totals[0]->TotalCount : 0 }}</span></p>
                <p class="mainheadtext">Matched: <span
                        class="tealh">{{ isset($_totals[0]->MatchedCount) ? $_totals[0]->MatchedCount : 0 }}</span>
                </p>
                <p class="mainheadtext">Not Matched: <span
                        class="lightcoralh">{{ isset($_totals[0]->NotMatchedCount) ? $_totals[0]->NotMatchedCount : 0 }}</span>
                </p>
            </div>
            ...
            <div class="d-flex flex-column flex-md-row justify-content-center gap-4 align-items-center mb-4">
                <div class="flex-main padding-top10">
                    ...
                    <span class="blackh">{{ isset($_totals[0]->sales_total) ? $_totals[0]->sales_total : 0 }}</span>
                </div>
                <div class="flex-main padding-top10">
                    <p class="mainheadtext">Deposit: </p>
                    <span
                        class="tealh">{{ isset($_totals[0]->collection_total) ? $_totals[0]->collection_total : 0 }}</span>
                </div>
                <div class="flex-main padding-top10">
                    <p class="mainheadtext">Store Response Entry: </p>
                    <span
                        class="bluetext">{{ isset($_totals[0]->adjustment_total) ? $_totals[0]->adjustment_total : 0 }}</span>
                </div>

                <div class="flex-main padding-top10">
                    <p class="mainheadtext">Difference[Tender - Deposit - Store Response]: </p>
                    <span
                        class="lightcoralh">{{ isset($_totals[0]->difference_total) ? $_totals[0]->difference_total : 0 }}</span>
                </div>
            </div>
        </div>
                    ...
                    <tr>
                        ...
                        <th class="left first-col sticky-col bggrey">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <i style="opacity: .5; font-size: 1.8em" wire:click="orderBy('mposDate')"
                        ...
                        <th class="left">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <i style="opacity: .5; font-size: 1.8em" wire:click="orderBy('depositSlipNo')"
                                    class="fa-solid @if ($orderBy == 'asc') fa-caret-up @else fa-caret-down @endif"></i>
                            </div>
                        </th>
                    ...
                    @foreach ($cashRecons as $data)
                        <tr>
                            ...
                            <td class="left" scope="row"
                                wire:key="fef19a476993669aa0730d30c7e549b204838979f0a4fd9202117d4d9f9e6cef_{{ rand() }}">
                                <input class="form-check-input" type="checkbox"
                                    value="{{ $data->CashTenderBkDrpUID }}" x-model="selection" />
                            </td>
                            <td class="left first-col sticky-col bgwhite">
                                {{ !$data->mposDate ? '' : Carbon\Carbon::parse($data->mposDate)->format('d-m-Y') }}
                            </td>
                            ...
                            <td class="left">
                                ...
                                @else
                                    <span class="greentext">{{ $data->cashBankStatus }}</span>
                                @endif
                            </td>
                            ...
                            <td class="right">
                                @if ($data->bank_cash_difference > 100 || $data->bank_cash_difference < -100)
                                    <span class="redtext">{{ $data->bank_cash_difference }}</span>
                                @else
                                    {{ $data->bank_cash_difference }}
                                @endif
                            </td>
                            ...
                            @if ($data->isUpdated == 0)
                                <td class="text-center"
                                    wire:key="0eecf827c42c9bc78e57344d511cb4b7877afae2b3db4d585a2684f341f91f73_{{ $data->CashTenderBkDrpUID }}">
                                    <a href="#" data-bs-toggle="modal"
                                        data-bs-target="#sdkcskskdsfkxnskdldkzsdsdlksddlskoijfdskvm_{{ $data->CashTenderBkDrpUID }}"><i
                                            class="fa-solid fa-eye"></i></a>
                                </td>
                            @else
                                <td class="text-center">
                                    <a href="#" data-bs-toggle="modal" style="color: gray"><i
                                            class="fa-solid fa-eye"></i></a>
                                </td>
                            @endif
                        </tr>
