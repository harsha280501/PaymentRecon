...
<div class="modal fade" id="exampleModalCenter_{{ $data->cardMisBankStRecoUID }}" tabindex="1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document" style="max-width:98%;" x-data="{
        ...
        submit(id) {
            ...
            if(parseFloat(saleSum) != parseFloat(this.sale)) {
        ...
        {{-- modal body --}}
        <x-app.commercial-team.process.bank-statement-process.upi-recon.popup-model :data="$data">
            <div class="modal-body" style="position: unset">
                <x-app.commercial-team.process.bank-statement-process.upi-recon.popup-headers :data="$data" />

                <h5>Reconciliation Window </h5>
                <x-app.commercial-team.process.bank-statement-process.upi-recon.recon-window :data="$data" />
                <br>
                {{-- Manual Entry --}}
                <h5>Entry By Store for reconciliation</h5>
                <div>
                    {{-- Add Process --}}
                    <x-app.commercial-team.process.bank-statement-process.upi-recon.add-recon-button ::recons="recons" />
                    <x-app.commercial-team.process.bank-statement-process.upi-recon.manual-entry :data="$data">
                        <template x-for="recon in recons">
                            <x-app.commercial-team.process.bank-statement-process.upi-recon.cash-process-entry :remarks="$remarks" ::recon="recon" :data="$data" />
                        </template>
                    </x-app.commercial-team.process.bank-statement-process.upi-recon.manual-entry>
                    <x-app.commercial-team.process.bank-statement-process.upi-recon.get-amount-totals :data="$data" />
                ...
                <x-app.commercial-team.process.bank-statement-process.upi-recon.history :data="$data" />
                <br>
            </div>
            {{-- modal footer --}}
            <x-app.commercial-team.process.bank-statement-process.upi-recon.popup-model-footer :data="$data">
            ...
            </x-app.commercial-team.process.bank-statement-process.upi-recon.popup-model-footer>
            {{-- Popup modal end --}}
        </x-app.commercial-team.process.bank-statement-process.upi-recon.popup-model>
    </div>
</div>
