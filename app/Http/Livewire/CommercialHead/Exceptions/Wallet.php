...
use App\Exports\CommercialHead\Exception\WalletExport;
...
class Wallet extends Component {
    ...
    public function render() {
        ...
        return view('livewire.commercial-head.exceptions.wallet', [
            'datas' => $dataset,
        ]);
    }
    ...
    public function export() {
        ...
        $dataset = DB::withOrderBySelect(
            'PaymentMIS_PROC_SELECT_COMMERCIALHEAD_Exception_wallet :procType, :bank, :from, :to',
            $params,
            $this->perPage,
            $this->orderBy
        );

        return Excel::download(new WalletExport(collect($dataset)), 'Payment_MIS_Exception_Wallet_' . now()->format('Y_m_d') . '.xlsx');
    }
