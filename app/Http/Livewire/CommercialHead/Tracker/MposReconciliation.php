...
class MposReconciliation extends Component implements UseExcelDataset, WithHeaders {
    ...
    public function headers(string $type = ''): array {
        ...
        if ($type == 'importable') {
            return [
                "UID", "Sales Date", "Deposit Date",
                "Store ID", "Retek Code", "Col Bank",
                "Status", "Deposit SlipNo", "Tender Amount", "Deposit Amount",
                "Store Response Entry", "Difference [Tender - Deposit - Store Response]", '',
                "New Sale Date", "New Store ID", "New Retek Code",
                "New Tender [ColBank]"
            ];
        }
        ...
        if (!$this->showBankDrop) {
            return [
                "Sales Date", "Deposit Date",
                "Store ID", "Retek Code", "Col Bank",
                "Status", "Deposit SlipNo", "Tender Amount",
        ...
        return [
            "Sales Date", "Deposit Date",
            "Store ID", "Retek Code", "Col Bank",
            "Status", "Bank Drop ID", "BankDrop Amount",
            "Deposit SlipNo", "Tender Amount", "Deposit Amount",
            "Store Response Entry", "Difference [Tender - Deposit - Store Response]"
        ]; 
    }
