...
class CardProcess extends Component implements UseExcelDataset, WithFormatting, WithHeaders
{
    ...
    public function viewCardSalesRecoUID($id)
    {
        $this->cardSalesRecoUID = (string) $id ?? '';
        $params = [
            'procType' => 'salesRecoUID',
            'startDate' => $this->startDate,
            'bank' => $this->bank,
            'location' => $this->Location,
            'endDate' => $this->endDate,
            'store' => $this->store,
            'id' => $this->cardSalesRecoUID,
        ];
        $this->showModal = true;
    ...
    public function searchBanks($search)
    {
        $this->filtering = true;

        return $this->executeSearch($search, 'card-banks');
    }
    public function searchStores($search)
    {
        $this->filtering = true;

        return $this->executeSearch($search, 'card-stores');
    }
    public function searchLocations($search)
    {
        $this->filtering = true;

        return $this->executeSearch($search, 'card-locations');
    }
    private function executeSearch($search, $type)
    {
        $params = [
            'procType' => $type,
