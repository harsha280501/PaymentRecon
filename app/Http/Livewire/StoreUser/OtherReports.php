...
class OtherReports extends Component implements UseExcelDataset, WithFormatting, WithHeaders {
    ...
    public function download(string $value = ''): Collection|bool {
        ...
        $params = [
            'procType' => 'simple-overall-summary-export',
    ...
    public function formatter(SpreadSheet $worksheet, $dataset): void {
        ...
        $worksheet->spreadsheet->getActiveSheet()->setCellValue('E' . 7, 'Sales');
        $worksheet->spreadsheet->getActiveSheet()->setCellValue('I' . 7, 'Balance');
        $worksheet->spreadsheet->getActiveSheet()->setCellValue('M' . 7, 'Collection');
        $worksheet->spreadsheet->getActiveSheet()->setCellValue('Q' . 7, 'Balance');
        $worksheet->spreadsheet->getActiveSheet()->mergeCells('E7:H7');
        $worksheet->spreadsheet->getActiveSheet()->mergeCells('I7:L7');
        $worksheet->spreadsheet->getActiveSheet()->mergeCells('M7:P7');
        $worksheet->spreadsheet->getActiveSheet()->mergeCells('Q7:T7');

        $worksheet->spreadsheet->getActiveSheet()->getStyle('A7:T7')->getBorders()->getAllBorders()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
