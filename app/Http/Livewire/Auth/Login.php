...
class Login extends Component {
    ...
    public function authenticate($request) {
        ...
        if ($pre_auth && $pre_auth->roleUID != 5 && ($pre_auth->isActive != '1')) {
            $this->addError('error', 'This Account is not active');
            return false;
        }  


        if ($pre_auth && $pre_auth->roleUID == 5 && ($pre_auth->isActive != '1' || $pre_auth->phase1Live != 1 || $pre_auth->hasLogin != 1)) {
