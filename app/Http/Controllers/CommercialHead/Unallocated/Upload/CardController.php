...
class CardController extends Controller
{
    ...
    public function importUnallocatedCard(Request $request)
    {
        $request->validate([
        ...
        $destinationPath = storage_path('app/public/commercial/unallocated/card/');
        ...
        $targetPath = storage_path('app/public/commercial/unallocated/card/') . $file_1_name;
            ...
            for ($i = 2; $i <= $arrayCount; $i++) {
                ...
                if (MFLInwardStoreIDMissingTransactions::where('storeMissingUID', $uniqueID)->exists()) {
                    ...
                    if ($store) {
                        ...
                        $collectionBank = trim(str_replace("'", '', $worksheet[$i]["G"])); //COllection Bank
                        $deposit_amount = ExcelUploadGeneralService::convertPriceFormatUsingNumeric(str_replace(',', '', $worksheet[$i]["H"])); // SLIP AMOUNT
                        ...
                        $bankModels = [
                            'sbi-card' => [MFLInwardCardMISSBIPos::class, 'CardMISSbiPosUID'],
                            'hdfc-card' => [MFLInwardCardMISHdfcPos::class, 'CardMISHdfcPosUID'],
                            'icici-card' => [MFLInwardCardMISIciciPos::class, 'CardMISIciciPosUID'],
                            'amex-card' => [MFLInwardCardMISAmexPos::class, 'CardMISAmexPosUID'],
                        ];
