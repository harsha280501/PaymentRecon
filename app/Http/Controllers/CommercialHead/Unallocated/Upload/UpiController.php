...
class UpiController extends Controller
{
    ...
    public function importUnallocatedUpi(Request $request)
    {
        $request->validate([
            ...
            for ($i = 2; $i <= $arrayCount; $i++) {
                ...
                if (MFLInwardStoreIDMissingTransactions::where('storeMissingUID', $uniqueID)->exists()) {
                    ...
                    if ($store) {
                        ...
                        $bankModels = [
                            'hdfc-upi' => MFLInwardUPIMISHdfcPos::class,
                            
                        ];

                        if (array_key_exists($collectionBankSlug, $bankModels)) {
                            $model = $bankModels[$collectionBankSlug];
                            $model::where('UPIMISHdfcPosUID',  $missingTransaction->UID)->update([
                                'storeID' => $storeID,
                                'retekCode' => $retekCode,
